                ...
                while (this.FlowRun)
                {
                    ...
                    switch (this.NowStep)
                    {
                        ...
                        case FlowStep.CalculateParabolicPeak:
                            {
                                if (TM.IsTimeOut())
                                {
                                    ...
                                    // ============ 保護機制 1: 檢查收斂條件 ============
                                    if (searchRange <= Min_Step)
                                    {
                                        this.info.General($"搜尋範圍已小於閾值 ({Min_Step })，準備收斂");
                                    ...
                                    if (!isMiddleHighest)
                                    {
                                        ...
                                        if (CurrentPoints.Y1 > CurrentPoints.Y3)
                                        {
                                            ...
                                            if (RetryCnt > 3)
                                            {
                                                this.info.Error($"搜尋範圍設置有誤, 建議小於 : {CurrentPoints.X1}");
                                        ...
                                        else
                                        {
                                            ...
                                            if (RetryCnt > 3)
                                            {
                                                this.info.Error($"搜尋範圍設置有誤, 建議大於 : {CurrentPoints.X3}");
                                                this.NowStep = FlowStep.Error;
                                            }
                                            else
                                            {
                        ...
                        case FlowStep.Error:
                            {
                                this.FlowRun = false;
                                this.info.General("Auto Focus Error !");
                            }
                            break;
