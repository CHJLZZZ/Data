...
namespace OpticalMeasuringSystem
{
    public class Task_AutoWD
    {
                    ...
                    case FlowStep.CalWD:
                        {
                            ...
                            //Current
                            double ImageX = CalResult.Block_Width;
        ...
        public ScreenRatioResult CalScreenRatio(MIL_ID Img, int Threshold, string SaveFolder, string Backup, bool SaveImage = false) //W,H
        {
            ...
            try
            {
                ...
                if (blobCount > 0)
                {
                    ...
                    MIL.MblobGetResult(blobResult, MIL.M_BLOB_INDEX(0), MIL.M_BOX_X_MIN, ref StartX);
                    MIL.MblobGetResult(blobResult, MIL.M_BLOB_INDEX(0), MIL.M_BOX_Y_MIN, ref StartY);
                    MIL.MblobGetResult(blobResult, MIL.M_BLOB_INDEX(0), MIL.M_BOX_X_MAX, ref EndX);
                    MIL.MblobGetResult(blobResult, MIL.M_BLOB_INDEX(0), MIL.M_BOX_Y_MAX, ref EndY);

                    int BlockW = (int)Math.Abs(EndX - StartX);
                    int BlockH = (int)Math.Abs(EndY - StartY);

                    Result.Block_X = (int)StartX;
                    Result.Block_Y = (int)StartY;
                    Result.ScreenRatio_W = (double)BlockW / (double)SizeX;
                    Result.ScreenRatio_H = (double)BlockH / (double)SizeY;
                    Result.Block_Width = BlockW;
                    Result.Block_Height = BlockH;

                    Rectangle ROI = new Rectangle 
                    {
                        X = Result.Block_X,
                        Y = Result.Block_Y,
                        Width = Result.Block_Width,
                        Height = Result.Block_Height,
                    };

                    DrawBlock?.Invoke(ROI);

                    if (SaveImage)
                    {
                        MIL.MbufSave($@"{SaveFolder}/{Backup}.tif", Img);
                    }
                }
                else
                {
        ...
        public double GetWD(MIL_ID Img,double RealX)
        {
            ScreenRatioResult CalResult = CalScreenRatio(Img, -1, "", "", false);
        ...
        {
            public int Block_X { get; set; } = 0;
            public int Block_Y { get; set; } = 0;

            public double ScreenRatio_H { get; set; } = 0.0;
            public double ScreenRatio_W { get; set; } = 0.0;
            public int Block_Width { get; set; } = 0;
            public int Block_Height { get; set; } = 0;
        }
