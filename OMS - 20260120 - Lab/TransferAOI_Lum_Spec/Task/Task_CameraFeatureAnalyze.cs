...
{
    public class Task_CameraFeatureAnalyze : TaskBase
    {
        private Task_AutoExpTime AutoExpTime;          // 狀態機-自動曝光

        public Task_CameraFeatureAnalyze(MilDigitizer grabber, IMotorControl motorControl, InfoManager info) : base(grabber, motorControl, info)
        {
        ...
        public override void MainFlow(object InputPara)
        {
            ...
            string OutputFolder = $@"{Para.OutputFolder}\{DateTime.Now.ToString("yyyyMMddHHmmss")}";
            ...
            while (FlowRun)
            {
                switch (FlowStep)
                {
                                    ...
                                    case Task_AutoExpTime.EnState.Finish:
                                        {
                                            ...
                                            string SavePath = $@"{OutputFolder}\{Gray_Index}.tif";
                                            MIL.MbufSave(SavePath, Grabber.grabImage);
                    ...
                    case (int)FlowState.Feature1_SaveResult:
                        {
                            string SavePath = $@"{OutputFolder}\Feature1.csv";
