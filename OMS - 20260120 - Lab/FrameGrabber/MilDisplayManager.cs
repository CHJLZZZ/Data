...
namespace FrameGrabber
{
	public class MilDisplayManager
	{
		...
		public void Init(bool MouseUse = true)
		{
		...
		#region --- SetWindow ---
		private delegate void SetWindowUICB(ref MIL_ID Img, ref Panel Pnl);
		...
		public void SetWindow(ref MIL_ID Img, ref Panel Pnl)
		{
			...
			{
				var d = new SetWindowUICB(SetWindow);
				Pnl.Invoke(d, new object[] { Img, Pnl });
			}
			else
			{
		...
		public bool AddROI(Rectangle ROI)
		{
			try
			{
				if (MilImage != MIL.M_NULL)
				{
					...
					MIL.MgraColor(ROI_MilGraphicsContext, MIL.M_COLOR_GREEN);

					MIL.MgraRect(ROI_MilGraphicsContext, ROI_MilGraphicsList, ROI.X, ROI.Y, ROI.X + ROI.Width, ROI.Y + ROI.Height);
		...
		public void DrawBlock(List<Rectangle> ROIs)
		{
			MIL.MgraClear(MIL.M_DEFAULT, Show_MilGraphicsList);
			...
			try
			{
				for (int i = 0; i < ROIs.Count; i++)
				{
					int S_X = ROIs[i].X;
					int S_Y = ROIs[i].Y;
					int E_X = ROIs[i].X + ROIs[i].Width;
					int E_Y = ROIs[i].Y + ROIs[i].Height;

					MIL.MgraRect(Show_MilGraphicsContext, Show_MilGraphicsList, S_X, S_Y, E_X, E_Y);
					MIL_INT graphicLabel = -1;
					MIL.MgraInquireList(Show_MilGraphicsList, MIL.M_LIST, MIL.M_DEFAULT, MIL.M_LAST_LABEL, ref graphicLabel);
					MIL.MgraControlList(Show_MilGraphicsList, MIL.M_GRAPHIC_LABEL(graphicLabel), MIL.M_DEFAULT, MIL.M_SELECTABLE, MIL.M_DISABLE);
				}
		...
		public void ZoomIn()
		{
			try
			{
				...
				// 沒影像就return
				if (MilDisplay == MIL.M_NULL) return;
				if (Viewer == null) return;
				if (MilImage == MIL.M_NULL) return;
				...
				oldZoom = oldZoom * 2;
				if (oldZoom == 0) oldZoom = 1;
				MIL.MdispZoom(MilDisplay, oldZoom, oldZoom);

				double centerX = (double)SizeX / 2.0;
				double centerY = (double)SizeY / 2.0;

				double panX = centerX - ((double)DisplayWidth / (2.0 * oldZoom));
				double panY = centerY - ((double)DisplayHeight / (2.0 * oldZoom));

				// 3. 強制定位
				MIL.MdispPan(MilDisplay, panX, panY);
			}
			catch
			{
		...
		public void CenterAlign()
		{
			...
			int DisplayWidth = Viewer.Size.Width;
			int DisplayHeight = Viewer.Size.Height;
		...
		public void ZoomOut()
		{
			try
			{
				...
				//if (oldZoom < ZoomFit) oldZoom = ZoomFit;
