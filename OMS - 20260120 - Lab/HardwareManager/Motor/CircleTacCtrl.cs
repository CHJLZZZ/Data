...
namespace HardwareManager
{
    public class CircleTacCtrl : IMotorControl
    {
        ...
        public CircleTacCtrl(InfoManager info)
        {
            this.info = info;
        }
        ...
        public void Open(bool ReadMcu, MotorSetting FocuserPara, MotorSetting AperturePara, MotorSetting FW1Para, MotorSetting FW2Para)
        {
            bool ReadMcuSucced = false;
            if (ReadMcu)
            {
                try
                {
                    MotorSetting Tmp_FocuserPara = new MotorSetting();
                    MotorSetting Tmp_AperturePara = new MotorSetting();
                    MotorSetting Tmp_FW1Para = new MotorSetting();
                    MotorSetting Tmp_FW2Para = new MotorSetting();

                    bool Rtn = ParaInit_ReadMcu(ref Tmp_FocuserPara, ref Tmp_AperturePara, ref Tmp_FW1Para, ref Tmp_FW2Para);

                    if (Rtn)
                    {
                        FocuserPara = Tmp_FocuserPara;
                        AperturePara = Tmp_AperturePara;
                        FW1Para = Tmp_FW1Para;
                        FW2Para = Tmp_FW2Para;
                        ReadMcuSucced = true;
                    }
                ...
                {
                    SaveLog($"Read Mcu Para Fail (Socket Error) : {ex.Message}", true);
                }
                catch (Exception ex)
                {
                    SaveLog($"Read Mcu Para Fail : {ex.Message}", true);
                }
            }

            try
            {
                bool Rtn = SetMcuPara(FocuserPara, AperturePara, FW1Para, FW2Para);

                if (Rtn)
                {
                    if (ReadMcuSucced)
                    {
                        SaveLog($"Set Mcu Para Succed (Read Mcu Setting)");
                    }
                    else
                    {
                        SaveLog($"Set Mcu Para Succed (Default Setting)");
                    }
                }
            ...
            {
                SaveLog($"Set Mcu Para Fail (Socket Error) : {ex.Message}", true);
            }
            catch (Exception ex)
            {
                SaveLog($"Set Mcu Para Fail : {ex.Message}", true);
            }
        }
        ...
        public bool ParaInit_ReadMcu(ref MotorSetting FocuserPara, ref MotorSetting AperturePara, ref MotorSetting FW1Para, ref MotorSetting FW2Para)
        {
            if (Doing) return true;
            ...
            try
            {
                ...
                FocuserPara = Focuser_Setting;
                AperturePara = Aperture_Setting;
                FW1Para = FW1_Setting;
                FW2Para = FW2_Setting;
