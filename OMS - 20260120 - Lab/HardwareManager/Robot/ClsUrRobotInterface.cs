...
namespace HardwareManager
{
	...
	public class ClsUrRobotInterface
	{
		...
		public ClsUrRobotInterface(InfoManager info)
		{
			this.Info = info;
		...
		private void SaveLog(string Msg, bool isAlarm = false)
		{
			if (isAlarm)
			{
				Info.Error($"[urRobot] {Msg}");
			}
			else
			{
				Info.General($"[urRobot] {Msg}");
			}
		}
		...
		private bool _isRobotMoveTcpOk(double x, double y, double z, double rx, double ry, double rz, int timeout)
		{
			double X_diff = 0.0;
			double Y_diff = 0.0;
			double Z_diff = 0.0;
			...
			try
			{
				...
				while (true)
				{
					...
					bool CheckX = X_diff < 3;
					bool CheckY = Y_diff < 3;
					bool CheckZ = Z_diff < 3;
					bool CheckRoll = Roll_diff < 0.5;
					bool CheckPitch = Pitch_diff < 0.5;
					bool CheckYaw = Yaw_diff < 0.5;

					if (CheckX && CheckY && CheckZ && CheckRoll && CheckPitch && CheckYaw)
					{
						break;
					}
		...
		private bool _isRobotMoveAngleOk(
			...
			int timeout)
		{
			try
			{
				...
				while (true)
				{
					...
					double X_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[0] - baseAngle);
					double Y_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[1] - shoulderAngle);
					double Z_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[2] - elbowAngle);
					double Roll_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[3] - wrist1Angle);
					double Pitch_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[4] - wrist2Angle);
					double Yaw_diff = Math.Abs(this.m_ClsUrStatus.MotorAngle[5] - wrist3Angle);

					bool CheckX = X_diff < 1;
					bool CheckY = Y_diff < 1;
					bool CheckZ = Z_diff < 1;
					bool CheckRoll = Roll_diff < 0.1;
					bool CheckPitch = Pitch_diff < 0.1;
					bool CheckYaw = Yaw_diff < 0.1;
			...
			try
			{
				...
				if (this.socket30003 != null)
				{
					if (this.socket30003.Connected)
					{
						...
						bRet = this._isRobotMoveTcpOk(x, y, z, rx, ry, rz, this.checkMoveStatusTimeOut);
		...
		public bool MoveJ_CustomDirection(double[] targetDegrees, int[] Dir, double acc = 1.4, double vel = 1.05, double blend = 0.0)
		{
			bool bRet = false;

			if (targetDegrees.Length != 6 || Dir.Length != 6) return false;

			double[] finalRadians = new double[6];

			for (int i = 0; i < 6; i++)
			{
				if (targetDegrees[i] == 0) return false;

				double current = this.m_ClsUrStatus.MotorAngle[i]; // 當前角度 (Degree)
				double target = targetDegrees[i];               // 目標角度 (Degree)

				// --- 正反轉邏輯處理 ---

				if (Dir[i] == -1)
				{
					// 如果使用者指定「反轉」，但目標值目前大於當前值
					// 我們減去 360 度，強迫它往負方向繞
					if (target > current)
					{
						target -= 360.0;
				...
				// 最後轉為 UR Script 用的弧度 (Radian)
				finalRadians[i] = target * Math.PI / 180.0;
			}

			string instruct = String.Format("movej([{0}, {1}, {2}, {3}, {4}, {5}], a = {6}, v = {7}, r = {8})\n",
				   finalRadians[0], finalRadians[1], finalRadians[2], finalRadians[3], finalRadians[4], finalRadians[5],
				   acc, vel, blend);


			if (this.socket30003 != null && this.socket30003.Connected)
			{
				this.socket30003.Send(Encoding.Default.GetBytes(instruct));

				this.isMoveing = true;

				bRet = this._isRobotMoveAngleOk(
					targetDegrees[0], targetDegrees[1], targetDegrees[2],
					targetDegrees[3], targetDegrees[4], targetDegrees[5],
					1000*10*6*10);
