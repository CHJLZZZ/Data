...
{
	MIL_ID		Mil_FunRtn = M_NULL;
	MIL_ID		CalibrationResult = M_NULL;
	...
	try
	{
		...
		McalSave(MIL_TEXT(SavePath), CalibrationResult, M_DEFAULT);
	}
	catch (Exception	^	ex)
	{
		throw ex;
	...
	finally
	{
		...
		{
			McalFree(CalibrationResult);
			CalibrationResult = M_NULL;
		}
	}
}
#pragma endregion
...
void ImgPreProcCore::FunCalCalbriationImg(MIL_ID SrcImg, MIL_ID DstImg, String ^ CoefPath)
{
	...
	{		
		...
		McalRestore(MIL_TEXT(CoefPath), this->MilSystem, M_DEFAULT, &CalibrationResult);
...
void ImgPreProcCore::FunCalFFCCoef(MIL_ID SrcImg, MIL_ID DstImg, String ^ CoefPath, MIL_ID OffsetImg, MIL_ID FlatImg, MIL_ID DarkImg)
{
	...
	try
	{
		...
		mImgFlatFieldContext = MimAlloc(this->MilSystem, M_FLAT_FIELD_CONTEXT, M_DEFAULT, M_NULL);
		...
		char		SavePath[DEMURA_MAX_PATH] = "\0";
		sprintf_s(SavePath, DEMURA_MAX_PATH, "%s", CoefPath);
		MbufSave(MIL_TEXT(SavePath), mImgCoef);
...
void ImgPreProcCore::FunCalFFCImg(MIL_ID SrcImg, MIL_ID DstImg, String ^ CoefPath)
{
	...
	try
	{
		char		SavePath[DEMURA_MAX_PATH] = "\0";
		sprintf_s(SavePath, DEMURA_MAX_PATH, "%s", CoefPath);
		MbufRestore(MIL_TEXT(SavePath), this->MilSystem, &mImgCoef);
...
void ImgPreProcCore::FunCalHotPixelImg(MIL_ID mImg, double HotPixel_Bright_TH, double HotPixel_Dark_TH, int HotPixel_CmpPitch, bool SaveCmpResult)
{
	...
	try
	{
		...
		if (SaveCmpResult)
		{
			...
			MbufSave(MIL_TEXT(SavePath), mImgHotPixelBin);
		}
	}
	catch (Exception	^	ex)
	{
	...
	finally
	{
		if (mAryHotPixelBin)
		{
			free(mAryHotPixelBin);
			mAryHotPixelBin = nullptr;
		}
